...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	\noindent 
	\textbf{Usability:} It must be easy for users to understand how to use the simulation capabilities. Jakob Nielsen's 10 general principles for interaction design [\ref{Nielsen}] should be considered and applied for implementation of user interface. For example, there should be a match between the system and real world so users understand the analogy. 
	...
	\begin{enumerate}
		...
		\item Traffic light's colour must change according to a certain delay associated with it by the global controller. The possible states are: RED, RED-AMBER, AMBER, GREEN. The exact behaviour of cars in these situations must be specified by a policy.
	...
	Although it would be very challenging to have our system be able to read information in a similar way, it would be useful to allow users to be able to specify the probability that a car will enter a given junction. 
	...
	The MVC uses the Notifies relation to connect instances of model, view and controller, i.e. elements are notified of a state change. 
...
\subsection{Project Timetabling}
	...
	All of the user actions are taken from the View and passed on to the controller for the respective actions to be taken. Effort has been made in making sure that any change in the status of the model is recorded by the application and is appropriate to be used wherever necessary in future developments.\\
...
Each time the car wants to move, it first checks whether or not its desired exit is straight in that direction. In our example, when the car first moves onto the roundabout it checks whether the north exit is any amount of cells north of it, however grass is to the north. The car must then check its surrounding radius of cells. At this point there are eight cases determining which direction the car should go. For our example, the car detects that there is circle (the roundabout island) to its west and road heading west to its east. This indicates that the car should move south so it calls the function moveSouth(). Next, the car again checks to see if the north exit is indeed north of it. Since it is not, it checks its surrounding radius of cells again. This check results in the circle being to the car's north-west and grass to its south-east, which indicates that the car should move south-west. This pattern continues for the other directional cases, moving the car around the roundabout. Once the car detects that the north exit is to its north, it moves north until it is on the northern road and then the rules for moving on a straight road apply.
  ...
  %% Toward the end of the project, we had another bump in the road regarding GitHub. When one person would update a document, GitHub would show that the document had been updated however it would not take the changes. This led to one group member writing a section of the paper that had already been written by another member. At another point, because the software was being continuously updated, GitHub was not allowing the coordinator to upload a version of the paper which delayed another team member from editing it. 
